...
- If the user gives no narrower scope, inspect the repository or current working area and clean up all high-confidence opportunities found, including assessing whether any files should be split.
- If the user names files, directories, globs, modules, or packages, treat that as the approved scope and perform full high-confidence cleanup inside it, including assessing whether any files should be split.
- If the user specifies an exact cleanup, follow that request instead of expanding into unrelated cleanup, but still assess and report file-splitting fit unless the user explicitly forbids it.
...
3. Map the code shape.
   ...
   - Always assess possible file splits across the approved scope; look for responsibility boundaries that improve locality, module depth, testability, dependency direction, or reviewability rather than only reducing line count.
